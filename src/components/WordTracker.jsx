...
export default function WordTracker(props) {
            ...
            <div className="winner" key={index}>
              <div className="winner-first-line">
                ...
                <p
                  style={{
                    color: `${
                      winner.userColor === null ? "#FF7F50" : winner.userColor
                    }`,
                  }}
                >
                  {winner.user}
  ...
  const winnersDisplay = (
    <div className="winners-container">
      ...
      <div className="winners-list">{winnerList}</div>
    </div>
  );
  ...
  const inputMethod = (
    <div className="check-method mx-4">
      ...
      <select
        ...
        name=""
        onChange={(e) => checkMethod(e.target.value)}
  ...
  const btnMulti = multiRound ? (
    <div className="d-flex w-50 justify-content-between">
      <div className="d-flex">
        <div className="btn mx-4" onClick={handleMulti}>
          Single round
        </div>
        <div className="btn mx-4" onClick={addScore}>
          Add score
        </div>
      </div>
      <div className="d-flex">
        <div className="btn" onClick={clearScore}>
          clear score
        </div>
      </div>
    </div>
  ) : (
    <div className="btn" onClick={handleMulti}>
      Multi Round
    </div>
  );
    ...
    <div className="word-tracker-container mb-4">
      ...
      <div className="d-flex">
        <input
          ...
          onChange={(event) => handleOriginal(event.target.value)}
          value={originalWord}
        />
        {inputMethod}
      </div>
